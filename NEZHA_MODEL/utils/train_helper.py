...
"""
 ...
 @Author  : chenmingwei (ma_cancan@163.com)
...
def get_args_parser():
    from NEZHA_TensorFlow.NEZHA_MODEL.model_train.nezha_lstm_ner import __version__
    parser = argparse.ArgumentParser()
    if os.name == 'nt':
        bert_path = 'F:\chinese_L-12_H-768_A-12'
        root_path = r'C:\workspace\python\BERT-BiLSTM-CRF-NER'
    else:
        bert_path = './NEZHA_MODEL/NEZHA-Base-WWM/'
        root_path = './'
    ...
    group1.add_argument('-init_checkpoint', type=str, default=os.path.join(bert_path, 'model.ckpt-691689'),
                        help='Initial checkpoint (usually from a pre-trained BERT model).')
    group1.add_argument('-vocab_file', type=str, default=os.path.join(bert_path, 'vocab.txt'),
                        help='')
    group1.add_argument('-partspeech_dict', type=str, default=os.path.join(root_path, "NERdata",'partspeech_dict.txt'),
                        help='Initial checkpoint (usually from a pre-trained BERT model).')

    group2 = parser.add_argument_group('Model Config', 'config the model params')
    group2.add_argument('-max_seq_length', type=int, default=400,
    ...
    group2.add_argument('-batch_size', type=int, default=1,
                        help='Total batch size for training, eval and predict.')
    group2.add_argument('-learning_rate', type=float, default=2e-5,
                        help='The initial learning rate for Adam.')
    group2.add_argument('-num_train_epochs', type=float, default=100,
                        help='Total number of training epochs to perform.')
    group2.add_argument('-dropout_rate', type=float, default=0.5,                      help='Dropout rate')
