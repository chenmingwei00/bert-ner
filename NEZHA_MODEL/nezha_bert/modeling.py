...
from NEZHA_TensorFlow.NEZHA_MODEL.nezha_bert.gpu_environment import get_custom_getter
...
def embedding_postprocessor(input_tensor,
  ...
  # with tf.variable_scope('partspeech_embeeding'):
  #
  #   speech_type_table = tf.get_variable(
  #       name="speech_embedding",
  #       shape=[part_type_vocab_size, width],
  #       initializer=create_initializer(initializer_range))
  #   # This vocab will be small so we always do one-hot here, since it is always
  #   # faster for a small vocabulary.
  #   flat_token_part_ids = tf.reshape(words_parts_ids, [-1])
  #   one_hot_ids = tf.one_hot(flat_token_part_ids, depth=part_type_vocab_size)
  #   token_speech_embeddings = tf.matmul(one_hot_ids, speech_type_table)
  #   token_speech_embeddings = tf.reshape(token_speech_embeddings,
  #                                        [batch_size, seq_length, width])
  #   output += token_speech_embeddings
...
def _generate_relative_positions_matrix(length, max_relative_position,
  ...
  if not cache:
    ...
    distance_mat = range_mat - tf.transpose(range_mat) #相对位置矩阵
  else:
    distance_mat = tf.expand_dims(tf.range(-length+1, 1, 1), 0)
  distance_mat_clipped = tf.clip_by_value(distance_mat, -max_relative_position,
                                          max_relative_position) #最多的相对位置是max_relative_position
